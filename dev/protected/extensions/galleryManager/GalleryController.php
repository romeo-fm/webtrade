...
{
    public $allowedTypes = array('image/png', 'image/jpeg');
    ...
    public function actionAjaxUpload($gallery_id = null)
    {
        ...
        if (!$model->setImage($imageFile->getTempName())) {
            $model->delete();
            header("Content-Type: application/json");
            echo CJSON::encode(
                array(
                    'error' => 'Не допустимое разрешение картинки. Должно быть не больше 1024х1024рх',
                    'status' => '505',
                ));
            die;
        }
